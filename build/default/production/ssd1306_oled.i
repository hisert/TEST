...
unsigned char ssd1306_buffer[32 * 128 / 8];
void (*ssd1306_oled_i2c_start)(void);
unsigned char(*ssd1306_oled_i2c_write)(unsigned char data);
unsigned char(*ssd1306_oled_i2c_stop)(void);
...
void SSH1306_OLED_ClearDisplay(void)
  {
  memset(ssd1306_buffer, 0, (128 * 32 / 8));
  }

void SSH1306_OLED_FillDisplay(void)
  {
  memset(ssd1306_buffer, 0xFF, (128 * 32 / 8));
  }

void SSH1306_OLED_InvertDisplay(unsigned char value)
  {
  if (value) ssd1306_command(0xA7);
  else ssd1306_command(0xA6);
  }
...
void SSH1306_OLED_DrawPixel(unsigned int x, unsigned int y, unsigned char color)
  ...
  switch (color)
    {
    case 1: ssd1306_buffer[x + (y / 8) * 128] |= (1 << (y & 7));
      break;
    case 0: ssd1306_buffer[x + (y / 8) * 128] &= ~(1 << (y & 7));
      break;
    case 2: ssd1306_buffer[x + (y / 8) * 128] ^= (1 << (y & 7));
      break;
    }
  }
...
void SSH1306_OLED_Image(const unsigned char *image)
  {
  unsigned int i;
  for (i = 0; i < (128 * 32 / 8); i++) ssd1306_buffer[i] = *(image + i);
  }
