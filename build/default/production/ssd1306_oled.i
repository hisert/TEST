...
unsigned char(*oled_i2c_write)(unsigned char data);
unsigned char(*oled_i2c_stop)(void);
...
void ssd1306_command(unsigned char command)
{
    ...
    oled_i2c_start();
    oled_i2c_write(0x3C << 1);;
    oled_i2c_write(controlx);;
...
void OLED_ClearDisplay(void)
{
    memset(buffer, 0, (128 * 32 / 8));
}

void OLED_FillDisplay(void)
{
    memset(buffer, 0xFF, (128 * 32 / 8));
}
...
{
    unsigned int i;
    for (i = 0; i < (128 * 32 / 8); i++) buffer[i] = *(image + i);
}
