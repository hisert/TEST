...
# 54 "thread.c"
unsigned char SYSTEM_FUNCT_RETURN = 0;

unsigned char SYSTEM_FUNCT(SIRALI_TIME_OUT_t *SIRALI_TIME, unsigned int TIME_TUTUCU_MS, unsigned char komut)
{
    if (komut == 3) (SIRALI_TIME->flag) = (SIRALI_TIME->flag) & ~(SIRALI_TIME->temp);
    if (komut == 1) {
        ...
        if (((SIRALI_TIME->flag)) == 0) {
            ...
            SYSTEM_FUNCT_RETURN = 3;
            return 3;
        }
        if (((SIRALI_TIME->flag)) == (SIRALI_TIME->temp)) {
            if (TIME_OUT_CHECK(&(SIRALI_TIME->time_out), TIME_TUTUCU_MS)) {
                (SIRALI_TIME->flag) = (SIRALI_TIME->flag) & ~(SIRALI_TIME->temp);
                SYSTEM_FUNCT_RETURN = 2;
                return 2;
            } else {
                SYSTEM_FUNCT_RETURN = 1;
                return 1;
            }
        }
    }
    if (komut == 2) (SIRALI_TIME->temp) = 0;
...
unsigned char THREAD_CHECK(THREAD_t *thread_t)
{
    ...
    if ((thread_t->flag) & 0x01) {
        ...
        if ((thread_t->flag) & 0x04) {
            ...
            if (((thread_t->flag) & 0x10) == 0) {
            }
            temp = thread_t->Funct();
            if (((thread_t->flag) & 0x08));
            else if (temp)(thread_t->flag) = (thread_t->flag) & ~0x01;
        }
    }
    return temp;
}
