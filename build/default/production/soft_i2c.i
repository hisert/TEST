...
void SOFT_I2C_TRIS_WRITE(unsigned char SdaOrSck, unsigned char onOroff)
{
    ...
    if (SdaOrSck == 1) SOFT_I2C_BIT_WRITE((SOFT_I2C_1.SDA.tris_adress), SOFT_I2C_1.SDA.index, onOroff);
    if (SdaOrSck == 0) SOFT_I2C_BIT_WRITE((SOFT_I2C_1.SCK.tris_adress), SOFT_I2C_1.SCK.index, onOroff);
}

void SOFT_I2C_LAT_WRITE(unsigned char SdaOrSck, unsigned char onOroff)
{

    if (SdaOrSck == 1) SOFT_I2C_BIT_WRITE((SOFT_I2C_1.SDA.lat_adress), SOFT_I2C_1.SDA.index, onOroff);
    if (SdaOrSck == 0) SOFT_I2C_BIT_WRITE((SOFT_I2C_1.SCK.lat_adress), SOFT_I2C_1.SCK.index, onOroff);
}

unsigned char SOFT_I2C_PORT_READ(unsigned char SdaOrSck)
{
    if (SdaOrSck == 1) return SOFT_I2C_BIT_READ(*(SOFT_I2C_1.SDA.port_adress), SOFT_I2C_1.SDA.index);
    if (SdaOrSck == 0) return SOFT_I2C_BIT_READ(*(SOFT_I2C_1.SCK.port_adress), SOFT_I2C_1.SCK.index);
}
...
unsigned char SOFT_I2C_WRITE(unsigned char data)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        if ((data << i)&0x80) SOFT_I2C_TRIS_WRITE(1, 1);
        else SOFT_I2C_TRIS_WRITE(1, 0);
...
unsigned char SOFT_I2C_READ()
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        RxData = (unsigned char) (RxData | (SOFT_I2C_PORT_READ(1) << (7 - i)));
...
unsigned char SOFT_I2C_READ_ACK()
{
    unsigned char data = SOFT_I2C_READ();
    SOFT_I2C_SEND_ACK();
    return data;
}
