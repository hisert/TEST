...
extern void WAIT_INTERRUPT(unsigned int ms);
...
void MENU_BUTON_ADD(char process)
  ...
  if (process == 'R') MENU_BUTON_STATUS = MENU_BUTON_STATUS | 0x10;
  }
...
void MENU_TEXT_PRINT(const char *text, unsigned char index)
  ...
  MENU_WRITE_TEXT(0, 20, msg);
  sprintf(msg, "%c/%c", (index + '1'), (MENU_MAX + '1'));
  MENU_WRITE_TEXT(105, 48, msg);
  MENU_UPDATE();
  }
...
void MENU_USE_VAL(const char *ust_yazi, unsigned int data_max, unsigned int data_min, unsigned int data)
  ...
  MENU_TEXT_ORTALA(ust_yazi, msg, 16);
  MENU_WRITE_TEXT(0, 0, msg);
  MENU_WRITE_TEXT(0, 24, "MAX   NOW    MIN");
  MENU_WRITE_DEC(0, 48, data_max);

  if (data_min >= 1000) MENU_WRITE_DEC(103, 48, data_min);
  else if (data_min >= 100) MENU_WRITE_DEC(109, 48, data_min);
  else MENU_WRITE_DEC(116, 48, data_min);

  if (data >= 1000) MENU_WRITE_DEC(51, 48, data);
  else if (data >= 100) MENU_WRITE_DEC(53, 48, data);
  else MENU_WRITE_DEC(56, 48, data);
  MENU_UPDATE();
  }
...
void MENU_WORK(char MENU_MOVEMENT)
  ...
  if (MENU_SHOW_FLAG == 0)
    ...
    if (MENU_MOVEMENT == 'O')
      ...
      else if ((MENU_POINTER[MENU_INDEX_COUNTER].flag & 0x02) || (MENU_POINTER[MENU_INDEX_COUNTER].flag & 0x04) || (MENU_POINTER[MENU_INDEX_COUNTER].flag & 0x10))
        {
        if (VAL_CHANGE_FLAG == 0)
          {
          MENU_TEMP_VAL = 0;
          ...
          if (MENU_POINTER[MENU_INDEX_COUNTER].Funct != 0)
            ...
            if ((MENU_POINTER[MENU_INDEX_COUNTER].flag & 0x10) == 0) MENU_TEXT_SAVED();
            WAIT_INTERRUPT(500);
            }
          VAL_CHANGE_FLAG = 0;
          }
...
void MENU_PROCESS()
  ...
  if (MENU_BUTON_STATUS)
    ...
    if (MENU_BUTON_STATUS & 0x10)
      {
      MENU_BUTON_STATUS = MENU_BUTON_STATUS & ~0x10;
      MENU_WORK('R');
      }
    }
  }
