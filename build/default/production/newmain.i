...
# 12 "./thread.h"
    ...
    unsigned char SYSTEM_FUNCT(SIRALI_TIME_OUT_t *SIRALI_TIME, unsigned int TIME_TUTUCU_MS, unsigned char komut);
    unsigned char SYSTEM_FUNCT_GET_STATE();
# 47 "./thread.h"
    typedef struct {
        unsigned char flag;
...
unsigned char THREAD_LED_FUNCT()
{
    ...
    SYSTEM_FUNCT(&timer, 0, 2);
    if (SYSTEM_FUNCT(&timer, 100, 1)) if (SYSTEM_FUNCT_GET_STATE() == 3) PIN_SET_LAT('D', 1, 'H');
    if (SYSTEM_FUNCT(&timer, 900, 1)) if (SYSTEM_FUNCT_GET_STATE() == 3) PIN_SET_LAT('D', 1, 'L');
    return 0x00;
}
...
void PROGRAM_1()
{
    ...
    SYSTEM_FUNCT(&timer, 0, 2);
    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 1)) PIN_SET_LAT_TOGGLE('D', 3);
    }

    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 2)) PIN_SET_LAT_TOGGLE('D', 3);
    }

    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 4)) PIN_SET_LAT_TOGGLE('D', 3);
    }

    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 8)) PIN_SET_LAT_TOGGLE('D', 3);
    }

    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 16)) PIN_SET_LAT_TOGGLE('D', 3);
    }

    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 32)) PIN_SET_LAT_TOGGLE('D', 3);
    }

    if (SYSTEM_FUNCT(&timer, 2000, 1)) {
        if (SYSTEM_FUNCT_GET_STATE() == 2);
        else if (SYSTEM_FUNCT_GET_STATE() == 3) TIME_OUT_RESET(&timer_temp);
        else if (SYSTEM_FUNCT_GET_STATE() == 1) if (TIME_OUT_CHECK(&timer_temp, 64)) PIN_SET_LAT_TOGGLE('D', 3);
    }
}
...
int main(void)
{
    ...
    for (unsigned char x = 0; x < 4; x++) PIN_SET_IO('D', 'O', 'D', x + 1, 'L');
