...
# 25 "./register.h"
    typedef struct {
        ...
        unsigned long max;
        unsigned long min;
        void (*funct)(unsigned long val, unsigned char komut);
    } REG_t;
    ...
    void REG_FUNC(unsigned char regId, unsigned long val, unsigned char komut);
    void REG_CREATE(unsigned char regId, void (*funct_t)(unsigned long, unsigned char), unsigned int flag_t, unsigned long val_t, unsigned long max_t, unsigned long min_t);
...
unsigned int MENU_FUNCT_ARM_MAS12(unsigned char komut, unsigned int data)
{
...
Menu_One ARM_SETTING_MENU[] = {
    ...
    {.text = "MAS12 STATUS", .flag = 0x10, .Funct = MENU_FUNCT_ARM_MAS12, .max = 0, .min = 0},
};
...
void MENU_BUTON_READ()
{
    ...
    if (temp) {
        if (temp == 0x01) if (BUTON_GET_TIME(&MENU_UP) > 25) if ((BUTON_GET_TIME(&MENU_UP) % 1) == 0) MENU_BUTON_ADD('U');
        if (temp == 0x02) if (BUTON_GET_TIME(&MENU_UP) < 20) MENU_BUTON_ADD('U');
    }
    temp = BUTON_PROCESS(&MENU_OK, PIN_GET_PORT('B', 6), 2, 1);
    if (temp) {
        if (temp == 0x01) if (BUTON_GET_TIME(&MENU_OK) == 25) MENU_BUTON_ADD('B');
        if (temp == 0x02) if (BUTON_GET_TIME(&MENU_OK) < 20) MENU_BUTON_ADD('O');
    }
    temp = BUTON_PROCESS(&MENU_DOWN, PIN_GET_PORT('B', 7), 2, 1);
    if (temp) {
        if (temp == 0x01) if (BUTON_GET_TIME(&MENU_DOWN) > 25) if ((BUTON_GET_TIME(&MENU_DOWN) % 1) == 0) MENU_BUTON_ADD('D');
        if (temp == 0x02) if (BUTON_GET_TIME(&MENU_DOWN) < 20) MENU_BUTON_ADD('D');
    }
}
...
void LED_THREAD(unsigned char threadIndex)
{
    ...
    if (THREAD_TIME_WAIT(&timer, 95)) if (THREAD_GET_STATE() == 0x01) PIN_SET_LAT_TOGGLE('D', 1);
    if (THREAD_TIME_WAIT(&timer, 5)) if (THREAD_GET_STATE() == 0x01) PIN_SET_LAT_TOGGLE('D', 1);
...
void TASK_MENU_PROCESS_FUNCT(unsigned char taskIndex)
{
...
void REG_PROCESS(char *msg)
{
    ...
    static char TX[50];
    static char DATA_S[10];
    static char SYMBOL_S[3];
    static char ADRESS_S[6];
    static char SECOND_ADRESS_S[3];
    static char KOMUT_S[3];
    static char READ_WRITE[2];
    static char temp[10];
...
passed:
    for (unsigned char x = 0; x < 50; x++) TX[x] = 0x00;
    ...
    if (REG_GET_FLAG(KOMUT) & 0x1000) UART_0_STRING(TX);
}
...
void INIT_REG()
{
    REG_CREATE(REGx_ADDR, 0, 0x1000 | 0x0001 | 0x0008 | 0x0200 | 0x0400 | 0x0800, 0, 0xFFFF, 0);
    REG_CREATE(REGx_RAND_CODE, 0, 0x1000 | 0x0001 | 0x0008 | 0x0200 | 0x0400, 0, 0xFFFF, 0);
    REG_CREATE(REGx_SERI_NO, 0, 0x1000 | 0x0001 | 0x0008 | 0x0200 | 0x0400 | 0x0800, 0, 0xFFFF, 0);
    REG_CREATE(REGx_VERSION, 0, 0x1000 | 0x0001 | 0x0008 | 0x0400, 0x10, 0xFFFF, 0);
    REG_CREATE(REGx_SLAVE_ADRESS, 0, 0x1000 | 0x0001 | 0x0004 | 0x0200 | 0x0400 | 0x0800, 0, 0xFF, 0);
...
int main(void)
{
    ...
    UART_0_INIT(115200);
# 716 "newmain.c"
    THREAD_CREATE(THREAD_LED_CANLI, 0x01 | 0x08 | 0x10, 10, LED_THREAD);
