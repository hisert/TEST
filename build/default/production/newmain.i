...
#pragma config EBTR7 = OFF
    ...
    void MCU_INIT(unsigned char Mhz);
    ...
    unsigned char PIN_INIT_PPS(char port, unsigned char pin);
    void PIN_INIT_IOC(char Port, unsigned char Pin, char proity, unsigned char openOrClose);
    ...
    void PWM_TIMER_8_INIT(unsigned long Hz);
    void PWM_1_INIT();
    ...
    void PWM_6_INIT();
    void PWM_6_DUTY(unsigned char duty);

    void UART_1_INTERRUPT(unsigned char OpenOrClose);
    void UART_1_INIT(unsigned int baudRate);
    ...
    void UART_2_INIT(unsigned int baudRate);
    void UART_2_BYTE(unsigned char out);
    void UART_2_STRING(char* text);
    void UART_2_DECIMAL(unsigned long val);
    void UART_3_INTERRUPT(unsigned char OpenOrClose);
    void UART_3_INIT(unsigned int baudRate);
    void UART_3_BYTE(unsigned char out);
    void UART_3_STRING(char* text);
    void UART_3_DECIMAL(unsigned long val);
    void UART_4_INTERRUPT(unsigned char OpenOrClose);
    void UART_4_INIT(unsigned int baudRate);
    void UART_4_BYTE(unsigned char out);
    void UART_4_STRING(char* text);
    void UART_4_DECIMAL(unsigned long val);
    void UART_5_INTERRUPT(unsigned char OpenOrClose);
    void UART_5_INIT(unsigned int baudRate);
    void UART_5_BYTE(unsigned char out);
    void UART_5_STRING(char* text);
    void UART_5_DECIMAL(unsigned long val);

    void UART_1_INTERRUPT_FUNCT_CONNECT(unsigned long baudrate, void(*UART_1_INTERRUPT_FUNCT_POINTER_t)(unsigned char));
    void UART_2_INTERRUPT_FUNCT_CONNECT(unsigned long baudrate, void(*UART_2_INTERRUPT_FUNCT_POINTER_t)(unsigned char));
    void UART_3_INTERRUPT_FUNCT_CONNECT(unsigned long baudrate, void(*UART_3_INTERRUPT_FUNCT_POINTER_t)(unsigned char));
    void UART_4_INTERRUPT_FUNCT_CONNECT(unsigned long baudrate, void(*UART_4_INTERRUPT_FUNCT_POINTER_t)(unsigned char));
    void UART_5_INTERRUPT_FUNCT_CONNECT(unsigned long baudrate, void(*UART_5_INTERRUPT_FUNCT_POINTER_t)(unsigned char));
    void TIMER_1_INTERRUPT_CONNECT(unsigned int ms, void(*TIMER_1_INTERRUPT_FUNCT_POINTER_t)(void));
    void TIMER_3_INTERRUPT_CONNECT(unsigned int ms, void(*TIMER_3_INTERRUPT_FUNCT_POINTER_t)(void));
    void TIMER_5_INTERRUPT_CONNECT(unsigned int ms, void(*TIMER_5_INTERRUPT_FUNCT_POINTER_t)(void));
# 7 "newmain.c" 2
# 1 "./PIC_18F46K22.h" 1
# 1 "./common.h" 1
...
# 12 "./thread.h"
    ...
    void THREAD_INIT(THREAD_t *thread_t, unsigned char flag, unsigned int duty_time, unsigned char(*Funct_t)(void));
    void THREAD_START(THREAD_t *thread_t);
    void THREAD_STOP(THREAD_t *thread_t);
    unsigned char THREAD_is_START(THREAD_t *thread_t);
    unsigned char THREAD_CHECK(THREAD_t *thread_t);
...
# 4 "./soft_i2c.h" 2
    ...
    void SOFT_I2C_INIT(volatile unsigned char *sda_tris_adress, volatile unsigned char *sda_lat_adress, volatile unsigned char *sda_port_adress, unsigned char sda_index, volatile unsigned char *sck_tris_adress, volatile unsigned char *sck_lat_adress, volatile unsigned char *sck_port_adress, unsigned char sck_index);
    void SOFT_I2C_START();
    void SOFT_I2C_STOP();
    ...
    unsigned char SOFT_I2C_READ_ACK();
    unsigned char SOFT_I2C_READ_NACK();
    unsigned char SOFT_I2C_TEST();
# 12 "newmain.c" 2
# 1 "./eeprom.h" 1
# 1 "./common.h" 1
...
# 35 "./MENU.h"
    ...
    void MENU_CIK();
    void MENU_TEXT_PRINT(const char *text, unsigned char index);
    void MENU_SHOW_DATA_PRINT(const char *text, unsigned int data);
    void MENU_USE_SELECT(const char *ust_yazi, const char *data[], unsigned char index);
    void MENU_USE_VAL(const char *ust_yazi, unsigned int data_max, unsigned int data_min, unsigned int data);
...
# 2 "./ssd1306_oled.h" 2
    ...
    void SSH1306_OLED_SetFont(const unsigned char *font);
    void SSH1306_OLED_InvertFont(unsigned char invert_status);
    void SSH1306_OLED_SetContrast(unsigned char contrast);
    void SSH1306_OLED_ClearDisplay(void);
    void SSH1306_OLED_FillDisplay(void);
    void SSH1306_OLED_InvertDisplay(unsigned char value);
    void SSH1306_OLED_DrawPixel(unsigned int x, unsigned int y, unsigned char color);
    void SSH1306_OLED_Line(unsigned int x_start, unsigned int y_start, unsigned int x_end, unsigned int y_end, unsigned char color);
    void SSH1306_OLED_V_Line(unsigned int y_start, unsigned int y_end, unsigned int x_pos, unsigned char color);
    void SSH1306_OLED_H_Line(unsigned int x_start, unsigned int x_end, unsigned int y_pos, unsigned char color);
    void SSH1306_OLED_Rectangle(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, unsigned char color);
    void SSH1306_OLED_FillRectangle(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, unsigned char color);
    void SSH1306_OLED_Triangle(unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, unsigned char color);
    void SSH1306_OLED_Image(const unsigned char *image);
    void SSH1306_OLED_Circle(unsigned int x0, unsigned int y0, unsigned int r, unsigned char color);
    void SSH1306_OLED_Write(unsigned int x, unsigned int y, char value);
    void SSH1306_OLED_Write_Text(unsigned int x, unsigned int y, const char *text);
...
# 30 "./AS5600_MAS.h"
    ...
    void MOTOR_TORK_SET(unsigned int tork);
...
# 2 "./ws2812b.h" 2
# 20 "./ws2812b.h"
    void LED_INIT_PORT(unsigned char *lat_adress_t, unsigned char index_t);
    void LED_CLEAN();
    void LED_UPDATE();
    void LED_DATA_COPY_SINGLE(unsigned int dataLen, unsigned long data);
    void LED_DATA_MASK(unsigned int dataStart, unsigned int dataLen, unsigned long data);
    void LED_DATA_REVERSE(unsigned int dataLen);
    void LED_TOGGLE_DATA(unsigned char index, unsigned int dataLen, unsigned long data, unsigned char fast);
    void LED_SLEEPING(unsigned char index, unsigned int dataLen, unsigned long data);
...
# 27 "./register.h"
    typedef struct {
        ...
        unsigned long max;
        unsigned long min;
        void (*funct)(unsigned char regId, unsigned char komut);
    } REG_t;



    void REGISTER_INIT(void (*eeprom_byte_write_t)(unsigned int adress, unsigned char data), unsigned char(*eeprom_byte_read_t)(unsigned int adress));
    unsigned long REG_GET(unsigned char regId);
    void REG_SET(unsigned char regId, unsigned long val);
    void REG_EEPROM_SET(unsigned char regId);
    void REG_EEPROM_GET(unsigned char regId);
    unsigned int REG_GET_FLAG(unsigned char regId);
    void REG_FUNC(unsigned char regId, unsigned char komut);

    void REG_CREATE(unsigned char regId, void (*funct_t)(unsigned char, unsigned char), unsigned int flag_t, unsigned long val_t, unsigned long max_t, unsigned long min_t);
    void REG_TX_ADD(char* input, const char* data);

    void REG_PARSE_DATA(const char* src, char* dst);
    void REG_PARSE_INDEX(const char* src, char* out, unsigned char start, unsigned char stop);
    unsigned char REG_PARSE_COMPARE(const char* str1, const char* str2);

    void REG_CHANGE_RANDCODE();
    unsigned int REG_GET_RANDCODE();
    void REG_SET_UNLOCK_START();
    void REG_SET_UNLOCK_STOP();
    unsigned char REG_GET_UNLOCK();

    unsigned long REG_CONVERT_STR_HEX_TO_DWORD(const char* str);
    unsigned long REG_CONVERT_STR_DEC_TO_DWORD(const char* str);
    void REG_CONVERT_DWORD_TO_STR_DEC(unsigned long value, char* out_str, unsigned char length);
...
unsigned char WAIT_WHILE_FLAG = 0;
...
Tone_t *PLAY_TONE = 0;
unsigned char BUZZER_PLAYING = 0;
...
unsigned char THREAD_UART_1_RX_FUNCT()
{
...
unsigned char THREAD_UART_2_RX_FUNCT()
{
    return 1;
}

unsigned char THREAD_LED_FUNCT()
{
    static THREAD_DELAY timer;
    THREAD_TIME_START(&timer);
    if (THREAD_TIME_WAIT(&timer, 1)) if (THREAD_GET_STATE() == 0x01) PIN_SET_LAT('B', 6, 'H');
    if (THREAD_TIME_WAIT(&timer, 9)) if (THREAD_GET_STATE() == 0x01) PIN_SET_LAT('B', 6, 'L');
    return 0;
}

unsigned char THREAD_BUZZER_FUNCT()
{
...
passed:
    ...
    if (REG_GET_FLAG(KOMUT) & 0x2000) RS485_SEND(TX);
}

void REG_FUNCT_MAS_HAM(unsigned char regId, unsigned char komut)
{
...
void INIT_REG()
{
    ...
    REG_CREATE(REGx_VERSION, 0, 0x2000 | 0x0001 | 0x0008 | 0x0400, 0x01, 0xFFFF, 0);
    REG_CREATE(REGx_SLAVE_ADRESS, 0, 0x2000 | 0x0001 | 0x0004 | 0x0200 | 0x0400 | 0x0800, 0, 0xFF, 0);

    REG_CREATE(REGx_MAS_HAM, REG_FUNCT_MAS_HAM, 0x2000 | 0x0002 | 0x0080 | 0x0400 | 0x1000, 0, 0xFFFF, 0);
...
int main(void)
{
    ...
    PIN_SET_IO('D', 'O', 'B', 7, 'L');
    PIN_SET_IO('D', 'O', 'F', 1, 'L');

    PIN_SET_IO('D', 'I', 'D', 0, 'L');
    PIN_SET_IO('D', 'I', 'H', 2, 'L');
    ...
    THREAD_INIT(&THREADS[THREAD_LED], 0x01 | 0x08, 100, THREAD_LED_FUNCT);
    ...
    THREAD_INIT(&THREADS[THREAD_BUZZER], 0x01 | 0x08, 0, THREAD_BUZZER_FUNCT);
