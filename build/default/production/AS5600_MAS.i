...
unsigned char(*mas_i2c_write)(unsigned char data);
unsigned char(*mas_i2c_read_ack)(void);
unsigned char(*mas_i2c_read_nack)(void);
...
signed int MAS12_REMAP(unsigned int middpoint, unsigned int index)
{
    ...
    if (index >= middpoint) {
        ...
        if (fark >= 2047) data = (4095 - fark) * -1;
        else data = fark;
    } else {
        fark = middpoint - index;
        if (fark >= 2047) data = (4095 - fark);
        else data = fark * -1;
    }
...
unsigned char MOTOR_WORK = 0;
unsigned char MOTOR_SUCCES = 0;
unsigned int MOTOR_STOP_INDEX = 0;
unsigned int MOTOR_START_INDEX = 0;
unsigned char MOTOR_KACIRMA_KURTARMA = 0;
unsigned char MOTOR_PWM_VAL = 0;

unsigned char MOTOR_DIRECTION = 0;
unsigned char MOTOR_MODE = 0;
unsigned int MAS12_NOW = 0;
unsigned int MAS12_NOW_MAPPED = 0;
unsigned int MOTOR_Z_POINT_INDEX = 0;
unsigned int MOTOR_TEKRAR_HAREKET_BASLAMA_TOLERANS = 0;
unsigned int MOTOR_TIME_OUT = 0;
unsigned int MOTOR_TIME_OUT_TEMP = 0;
unsigned int MOTOR_LAST_SUCCESS_INDEX = 2047;
unsigned int MOTOR_TORK_SINIR = 0;

unsigned char MOTOR_HIZ = 0;
unsigned char MOTOR_HIZ_MAX = 0;

unsigned char PID_MIN_PWM = 5;
unsigned char PID_MAX_PWM = 100;
float PID_INTEGRATED = 0;
float PID_Kp = 0;
float PID_Ki = 0;

void(*motor_pwm_w)(unsigned char pwm);
void(*motor_pwm_v)(unsigned char pwm);
void(*motor_pwm_u)(unsigned char pwm);
...
unsigned char MAS12_UPDATE()
{
    static unsigned int MAPPED_OLD = 0;
    unsigned int TEMP = MAS12_READ_HAL_DATA();
...
void MOTOR_PWM_INIT(void(*motor_pwm_w_t)(unsigned char), void(*motor_pwm_v_t)(unsigned char), void(*motor_pwm_u_t)(unsigned char))
{
...
void MOTOR_PWM(unsigned char MOTOR_DIRECTION, unsigned char pwm_value)
{
    if (MOTOR_MODE == 0) {
    ...
    if (MOTOR_MODE == 1) MOTOR_PWM_LOAD_ALL(pwm_value);
}

unsigned char MOTOR_LOOKING_STOP()
{
    if (MOTOR_DIRECTION == 1) if (MAS12_NOW_MAPPED > MOTOR_STOP_INDEX) return 1;
    if (MOTOR_DIRECTION == 0) if (MAS12_NOW_MAPPED < MOTOR_STOP_INDEX) return 1;
    return 0;
}

unsigned char MOTOR_PID_CONTROL()
{
    unsigned int kalan = abs(MAS12_NOW_MAPPED - MOTOR_STOP_INDEX);
    unsigned int all = abs(MOTOR_START_INDEX - MOTOR_STOP_INDEX);
    static unsigned char tekrar_kalkma_counter = 0;
    float Setpoint = kalan / 50;
...
void MOTOR_INIT(unsigned char c)
{
    MOTOR_MODE = c;
}
...
unsigned char MOTOR_START(unsigned int stop_index)
{
    ...
    if (MOTOR_MODE == 0) {
        if (MAS12_NOW_MAPPED > MOTOR_STOP_INDEX) MOTOR_DIRECTION = 0;
...
unsigned char MOTOR_PROCESS(unsigned int torkVal)
{
    unsigned char rtrnval = 0x00;
    unsigned char stopFlag = 0;
    unsigned char Mas_err_flag = MAS12_UPDATE();
    if (Mas_err_flag) rtrnval = 0x01;
    if (MOTOR_WORK) {
        if (Mas_err_flag) {
            MOTOR_SUCCES = 0;
            stopFlag = 1;
...
void FIRCASIZ_STEP(unsigned char Hall)
{
    PORTD = PORTD & ~(128);
    PORTD = PORTD & ~(128);
    PORTD = PORTD & ~(128);
...
void FIRCASIZ_DON(unsigned char Hall)
{
    if (MOTOR_DIRECTION == 1) {
        if (Hall == 5) Hall = 0;
...
void FIRCASIZ_READ(unsigned char HU, unsigned char HV, unsigned char HW)
{
    ...
    MOTOR_KACIRMA_KURTARMA = 10;
    unsigned char Hall = 0;
    if (HU) ((Hall) |= (1 << (0)));
    if (MOTOR_DIRECTION == 1) {
        if (HV) ((Hall) |= (1 << (1)));
        if (HW) ((Hall) |= (1 << (2)));
