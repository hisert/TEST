...
void (*eeprom_byte_write)(unsigned int adress, unsigned char data);
unsigned char(*eeprom_byte_read)(unsigned int adress);
...
void EEPROM_WRITE_DWORD(unsigned int address, unsigned long value)
{
    for (int i = 0; i < 4; i++) {
        unsigned char byte_value = (value >> (8 * i)) & 0xFF;
        eeprom_byte_write(address + i, byte_value);
    }
}

unsigned long EEPROM_READ_DWORD(unsigned int address)
{
    unsigned long value = 0;
    for (int i = 0; i < 4; i++) {
        unsigned char byte_value = eeprom_byte_read(address + i);
        value |= ((unsigned long) byte_value) << (8 * i);
    }
    return value;
}
