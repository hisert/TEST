...
unsigned long ws2812_data[100];
unsigned int ws812_data_counter = 0;




void ws2812_print(unsigned long color)
  {
  unsigned long data;
  cli();
...
void LED_CLEAN()
  {
  for (unsigned int x = 0; x < 50; x++) ws2812_data[x] = 0;
  ws812_data_counter = 0;
  }

void LED_UPDATE()
  {
  for (unsigned int x = 0; x < 50; x++) ws2812_print(ws2812_data[x]);
  ws812_data_counter = 0;
  }
...
void LED_DATA_MASK(unsigned int dataStart, unsigned int dataLen, unsigned long data)
  {
  for (unsigned int x = 0; x < dataLen; x++) ws2812_data[dataStart + x ] = data;
  }
...
void LED_DATA_REVERSE(unsigned int dataLen)
  ...
  dataLen = dataLen - 1;
  ws812_data_counter--;
  for (unsigned char x = 0; x <= dataLen / 2; x++)
    {
...
void rotate(unsigned long arr[], int n, int amount)
  {
...
void LED_TOGGLE_DATA(unsigned char index, unsigned int dataLen, unsigned long data, unsigned char fast)
  {
  static unsigned char fast_counter[10];
  static unsigned char toggle_flag[10];
  if (index == 0xFF)
    {
...
void LED_SLEEPING(unsigned char index, unsigned int dataLen, unsigned long data)
  ...
  static unsigned char pwm_holder[10];
  static unsigned char pwm_direction[10];
  if (index == 0xFF)
    {
    for (unsigned char x = 0; x < 10; x++)
      {
  ...
  unsigned long printdata = 0;
  unsigned char pwm_max = 255;
  unsigned char pwm_min = 20;
  unsigned char pwm_artis = 3;
  unsigned char pwm_azalis = 3;
  ...
  if (data == 0x0000FF) printdata = printdata;
  else if (data == 0x00FF00) printdata = printdata << 8;
  else if (data == 0xFF0000) printdata = printdata << 16;
  else if (data == 0x00FF00 + 0x0000FF) printdata = (printdata) + (printdata << 8);
  else if (data == 0x00FF00 + 0xFF0000) printdata = (printdata << 8) + (printdata << 16);
  else if (data == 0xFF0000 + 0x0000FF) printdata = (printdata) + (printdata << 16);
  else if (data == 0xFFFFFF) printdata = printdata + (printdata << 8) + (printdata << 16);
...
void LED_SONSUZ_TUR(unsigned char index, unsigned int dataLen, unsigned char fast)
  {
  static unsigned char fast_counter[10];
  static unsigned int kayma_place[10];
  if (index == 0xFF)
    {
    for (unsigned char x = 0; x < 10; x++)
      {
...
void LED_DATA_METEOR_EKLE(unsigned int ledsayisi, unsigned long data)
  ...
  for (signed int z = 0; z < ledsayisi; z++)
    ...
    if (data == 0x0000FF) printdata = printdata;
      ...
      if (data == 0xFFFFFF) printdata = printdata + (printdata << 8) + (printdata << 16);
