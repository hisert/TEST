...
void REGISTER_INIT(void (*eeprom_byte_write_t)(unsigned int adress, unsigned char data), unsigned char(*eeprom_byte_read_t)(unsigned int adress))
{
    ...
    if (EEPROM_GET(25 + 1) != 0xABCD) {
        for (unsigned char regId = 0; regId < 25; regId++) if ((REGS[regId].flag) & 0x0800) REG_EEPROM_SET(regId);
        EEPROM_SET(25 + 1, 0xABCD);
    }
    for (unsigned char regId = 0; regId < 25; regId++) if ((REGS[regId].flag) & 0x0800) REG_EEPROM_GET(regId);
}
