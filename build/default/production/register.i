...
unsigned int RANDCODE = 0;
unsigned char UNLOCK_STARTED = 0;
REG_t REGS[10];
# 307 "register.c"
void REGISTER_INIT(void (*eeprom_byte_write_t)(unsigned int adress, unsigned char data), unsigned char(*eeprom_byte_read_t)(unsigned int adress))
{
    EEPROM_INIT(eeprom_byte_write_t, eeprom_byte_read_t);
    if (EEPROM_GET(10 + 1) != 0xABCD) {
        for (unsigned char regId = 0; regId < 10; regId++) if ((REGS[regId].flag) & 0x0800) REG_EEPROM_SET(regId);
        EEPROM_SET(10 + 1, 0xABCD);
    }
    for (unsigned char regId = 0; regId < 10; regId++) if ((REGS[regId].flag) & 0x0800) REG_EEPROM_GET(regId);
}
...
void REG_CONVERT_DWORD_TO_STR_DEC(unsigned long value, char* out_str, unsigned char length)
{
    ...
    if (length <= val_len) {
        ...
        unsigned char pad = length - val_len;
...
void REG_PROCESS_GET_DATA_STR(unsigned char regId, char* out_str)
{
    if (REGS[regId].flag & 0x0004) REG_CONVERT_DWORD_TO_STR_HEX(REGS[regId].val, out_str, 2);
    if (REGS[regId].flag & 0x0008) REG_CONVERT_DWORD_TO_STR_HEX(REGS[regId].val, out_str, 4);
    if (REGS[regId].flag & 0x0010) REG_CONVERT_DWORD_TO_STR_DEC(REGS[regId].val, out_str, 1);
    if (REGS[regId].flag & 0x0020) REG_CONVERT_DWORD_TO_STR_DEC(REGS[regId].val, out_str, 2);
    if (REGS[regId].flag & 0x0040) REG_CONVERT_DWORD_TO_STR_DEC(REGS[regId].val, out_str, 3);
    if (REGS[regId].flag & 0x0080) REG_CONVERT_DWORD_TO_STR_DEC(REGS[regId].val, out_str, 4);
    if (REGS[regId].flag & 0x0100) REG_CONVERT_DWORD_TO_STR_DEC(REGS[regId].val, out_str, 5);
}

unsigned long REG_PROCESS_GET_DATA_DEC(unsigned char regId, const char* str)
{
    if (REGS[regId].flag & 0x0001) return REG_CONVERT_STR_HEX_TO_DWORD(str);
    if (REGS[regId].flag & 0x0002) return REG_CONVERT_STR_DEC_TO_DWORD(str);
}
...
void REG_CHANGE_RANDCODE()
{
    if (UNLOCK_STARTED) {
        do {
            RANDCODE = ((unsigned int) (rand())&0xFFFF);
        } while (!(RANDCODE));
    }
}
