...
void PIN_INIT_IOC(char Port, unsigned char Pin, char proity, unsigned char openOrClose)
{
    ...
    if (proity == 'P') {
        if (Port == 'B') {
            if (openOrClose == 1) IOCBP = IOCBP | hex;
            else if (openOrClose == 0) IOCBP = IOCBP & ~hex;
        } else if (Port == 'C') {
            if (openOrClose == 1) IOCCP = IOCCP | hex;
            else if (openOrClose == 0) IOCCP = IOCCP & ~hex;
        } else if (Port == 'E') {
            if (openOrClose == 1) IOCEP = IOCEP | hex;
            else if (openOrClose == 0) IOCEP = IOCEP & ~hex;
        } else if (Port == 'G') {
            if (openOrClose == 1) IOCGP = IOCGP | hex;
            else if (openOrClose == 0) IOCGP = IOCGP & ~hex;
        }
    } else if (proity == 'N') {
        if (Port == 'B') {
            if (openOrClose == 1) IOCBN = IOCBN | hex;
            else if (openOrClose == 0) IOCBN = IOCBN & ~hex;
        } else if (Port == 'C') {
            if (openOrClose == 1) IOCCN = IOCCN | hex;
            else if (openOrClose == 0) IOCCN = IOCCN & ~hex;
        } else if (Port == 'E') {
            if (openOrClose == 1) IOCEN = IOCEN | hex;
            else if (openOrClose == 0) IOCEN = IOCEN & ~hex;
        } else if (Port == 'G') {
            if (openOrClose == 1) IOCGN = IOCGN | hex;
            else if (openOrClose == 0) IOCGN = IOCGN & ~hex;
        }
    }
    if (PIE0bits.IOCIE == 0) PIE0bits.IOCIE = 1;
}
...
void PIN_INIT_WPU(char Port, unsigned char Pin, unsigned char openOrClose)
{
    unsigned char hex = (unsigned char) (1 << Pin);
    if (Port == 'A') {
        if (openOrClose == 1) WPUA = WPUA | hex;
        else if (openOrClose == 0) WPUA = WPUA & ~hex;
    } else if (Port == 'B') {
        if (openOrClose == 1) WPUB = WPUB | hex;
        else if (openOrClose == 0) WPUB = WPUB & ~hex;
    } else if (Port == 'C') {
        if (openOrClose == 1) WPUC = WPUC | hex;
        else if (openOrClose == 0) WPUC = WPUC & ~hex;
    } else if (Port == 'D') {
        if (openOrClose == 1) WPUD = WPUD | hex;
        else if (openOrClose == 0) WPUD = WPUD & ~hex;
    } else if (Port == 'E') {
        if (openOrClose == 1) WPUE = WPUE | hex;
        else if (openOrClose == 0) WPUE = WPUE & ~hex;
    } else if (Port == 'F') {
        if (openOrClose == 1) WPUF = WPUF | hex;
        else if (openOrClose == 0) WPUF = WPUF & ~hex;
    } else if (Port == 'G') {
        if (openOrClose == 1) WPUG = WPUG | hex;
        else if (openOrClose == 0) WPUG = WPUG & ~hex;
    } else if (Port == 'H') {
        if (openOrClose == 1) WPUH = WPUH | hex;
        else if (openOrClose == 0) WPUH = WPUH & ~hex;
    }
    PIE0bits.IOCIE = 1;
}
...
void TIMER_1_DUTY(unsigned char startOrStop)
{
...
void TIMER_1_INIT(unsigned int ms)
{
    ...
    for (unsigned char x = 0; x < 4; x++) {
        calculator = ((64000000UL / 1000) / (4 * prescale));
    ...
    temp = (unsigned int) calculator;
    temp = (unsigned int) (65536 - temp);
    PIR5bits.TMR1IF = 0;
    TMR1 = temp;
    TIMER_1_TICKS = temp;
    TIMER_1_DUTY(1);
...
void TIMER_3_DUTY(unsigned char startOrStop)
{
...
void TIMER_5_DUTY(unsigned char startOrStop)
{
...
void TIMER_7_INTERRUPT(unsigned char OpenOrClose)
{
    if (OpenOrClose == 1) PIE5bits.TMR7IE = 1;
    else if (OpenOrClose == 0) PIE5bits.TMR7IE = 0;
}

void TIMER_7_DUTY(unsigned char startOrStop)
{
...
void PWM_TIMER_2_INIT(unsigned long Hz)
{
    ...
    T2PR = (unsigned char) (calculator - 1);
    PWM_TIMER_2_SET(1);
}
...
void PWM_TIMER_4_INIT(unsigned long Hz)
{
    ...
    for (unsigned char x = 0; x < 8; x++) {
        calculator = 64000000UL / (prescale * Hz);
        if ((unsigned int) (calculator) <= 256)
            break;
        else {
            prescale *= 2;
            T4CONbits.CKPS = x;
    ...
    T4PR = (unsigned char) (calculator - 1);
    PWM_TIMER_4_SET(1);
}
...
void PWM_TIMER_6_INIT(unsigned long Hz)
{
    ...
    T6PR = (unsigned char) (calculator - 1);
    PWM_TIMER_6_SET(1);
}



void PWM_TIMER_8_SET(unsigned char openOrClose)
{
    if (openOrClose) T8CONbits.ON = 1;
    else T8CONbits.ON = 0;
}
...
void PWM_TIMER_8_INIT(unsigned long Hz)
{
    ...
    T8PR = (unsigned char) (calculator - 1);
    PWM_TIMER_8_SET(1);
}
...
void PWM_1_DUTY(unsigned char duty)
{
    ...
    CCPR1H = (unsigned char) ((temp) >> 8);
    CCPR1L = (unsigned char) (temp & 0xFF);
}
...
void PWM_2_DUTY(unsigned char duty)
{
    ...
    CCPR2H = (unsigned char) ((temp) >> 8);
    CCPR2L = (unsigned char) (temp & 0xFF);
}
...
void PWM_3_DUTY(unsigned char duty)
{
    ...
    CCPR3H = (unsigned char) ((temp) >> 8);
    CCPR3L = (unsigned char) (temp & 0xFF);
}
...
void PWM_4_DUTY(unsigned char duty)
{
    ...
    CCPR4H = (unsigned char) ((temp) >> 8);
    CCPR4L = (unsigned char) (temp & 0xFF);
}
...
void PWM_5_DUTY(unsigned char duty)
{
    ...
    CCPR5H = (unsigned char) ((temp) >> 8);
    CCPR5L = (unsigned char) (temp & 0xFF);
}
# 732 "PIC_18F67K40.c"
void UART_1_INTERRUPT(unsigned char OpenOrClose)
{
...
void UART_1_INIT(unsigned int baudRate)
{
    unsigned char bitLong = 8;
    unsigned long long calculater;
    RX1PPS = PIN_INIT_PPS('C', 6);
    RC7PPS = PPS_EUSART1_TX;
    PIN_SET_IO('D', 'I', 'C', 6, 'H');
    PIN_SET_IO('D', 'O', 'C', 7, 'H');
    RCSTA1bits.RX9 = 0;
    TXSTA1bits.TX9 = 0;

    calculater = 64000000UL / 64;
    calculater = calculater / (baudRate);
    SP1BRG = (unsigned char) (calculater - 1);
    TXSTA1bits.TXEN = 1;
    RCSTA1bits.CREN = 1;
    RCSTA1bits.SPEN = 1;
    UART_1_INTERRUPT(1);
...
{
    unsigned long long calculater;
    RX2PPS = PIN_INIT_PPS('G', 2);
    RG1PPS = PPS_EUSART2_TX;
    ...
    calculater = 64000000UL / 64;
    calculater = calculater / (baudRate);
    SP2BRG = (unsigned char) (calculater - 1);
    TXSTA2bits.TXEN = 1;
    RCSTA2bits.CREN = 1;
    RCSTA2bits.SPEN = 1;
    UART_2_INTERRUPT(1);
...
void UART_3_INIT(unsigned int baudRate)
{
    ...
    unsigned long long calculater;
    RX3PPS = PIN_INIT_PPS('E', 3);
    RE7PPS = PPS_EUSART3_TX;
    ...
    calculater = 64000000UL / 64;
    calculater = calculater / (baudRate);
    SP3BRG = (unsigned char) (calculater - 1);
    TXSTA3bits.TXEN = 1;
    RCSTA3bits.CREN = 1;
    RCSTA3bits.SPEN = 1;
    UART_3_INTERRUPT(1);
...
{
    unsigned long long calculater;
    RX4PPS = PIN_INIT_PPS('B', 2);
    RB4PPS = PPS_EUSART4_TX;
    ...
    calculater = 64000000UL / 64;
    calculater = calculater / (baudRate);
    SP4BRG = (unsigned char) (calculater - 1);
    TXSTA4bits.TXEN = 1;
    RCSTA4bits.CREN = 1;
    RCSTA4bits.SPEN = 1;
    UART_4_INTERRUPT(1);
...
void UART_5_INIT(unsigned int baudRate)
{
    ...
    RX5PPS = PIN_INIT_PPS('E', 4);
    RE6PPS = PPS_EUSART5_TX;
    ...
    calculater = 64000000UL / 64;
    calculater = calculater / (baudRate);
    SP5BRG = (unsigned char) (calculater - 1);
    TXSTA5bits.TXEN = 1;
    RCSTA5bits.CREN = 1;
    RCSTA5bits.SPEN = 1;
    UART_5_INTERRUPT(1);
...
void(*UART_1_INTERRUPT_FUNCT_POINTER)(unsigned char);
void(*UART_2_INTERRUPT_FUNCT_POINTER)(unsigned char);
void(*UART_3_INTERRUPT_FUNCT_POINTER)(unsigned char);
void(*UART_4_INTERRUPT_FUNCT_POINTER)(unsigned char);
void(*UART_5_INTERRUPT_FUNCT_POINTER)(unsigned char);
