...
# 2 "./ssh1106_oled.h" 2
    ...
    void ssh1106_command(unsigned char command);
    void ssh1106_data(unsigned char value);
    void SSH1106_OLED_SetFont(const unsigned char *font);
    void SSH1106_OLED_SetDisplayNormal(void);
    void SSH1106_OLED_SetDisplayInverted(void);
    void SSH1106_OLED_SetContrast(unsigned char contrast);
    void SSH1106_OLED_ClearDisplay(void);
    void SSH1106_OLED_FillDisplay(void);
    void SSH1106_OLED_InvertDisplay(unsigned char value);
    void SSH1106_OLED_DrawPixel(unsigned int x, unsigned int y, unsigned char color);
    void SSH1106_OLED_Line(unsigned int x_start, unsigned int y_start, unsigned int x_end, unsigned int y_end, unsigned char color);
    void SSH1106_OLED_V_Line(unsigned int y_start, unsigned int y_end, unsigned int x_pos, unsigned char color);
    void SSH1106_OLED_H_Line(unsigned int x_start, unsigned int x_end, unsigned int y_pos, unsigned char color);
    void SSH1106_OLED_Rectangle(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, unsigned char color);
    void SSH1106_OLED_FillRectangle(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, unsigned char color);
    void SSH1106_OLED_Triangle(unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, unsigned char color);
    void SSH1106_OLED_Image(const unsigned char *image);
    void SSH1106_OLED_Circle(unsigned int x0, unsigned int y0, unsigned int r, unsigned char color);
    void SSH1106_OLED_Write(unsigned int x, unsigned int y, char value);
    void SSH1106_OLED_Write_Text(unsigned int x, unsigned int y, const char *text);
...
const unsigned char font_8x16BOLD[] = {
                              0x08, 0x10, 0x20, 0x5f,
...
const unsigned char font_6x8[] = {
                         ...
                         0x00, 0xE7, 0xA7, 0xFF, 0xA0, 0xE0,
                         0x00
  };

const unsigned char Came_Logo [] = {
...
unsigned char ssh1106_buffer[64 * 128 / 8];

void (*ssh1106_oled_i2c_start)(void);
unsigned char(*ssh1106_oled_i2c_write)(unsigned char data);
unsigned char(*ssh1106_oled_i2c_stop)(void);
...
void ssh1106_command(unsigned char command)
  ...
  unsigned char controlx = 0x00;
  ssh1106_oled_i2c_start();
  ssh1106_oled_i2c_write(0x3C << 1);
...
void SSH1106_OLED_ClearDisplay(void)
  ...
  memset(ssh1106_buffer, 0, (128 * 64 / 8));
  }

void SSH1106_OLED_FillDisplay(void)
  {

  memset(ssh1106_buffer, 0xFF, (128 * 64 / 8));
  }
...
void SSH1106_OLED_Line(unsigned int x_start, unsigned int y_start, unsigned int x_end, unsigned int y_end, unsigned char color)
  ...
  signed long P;
  unsigned int i;
  dx = abs((unsigned int) (x_end - x_start));
  dy = abs((unsigned int) (y_end - y_start));
  x = x_start;
  y = y_start;
...
void SSH1106_OLED_Image(const unsigned char *image)
  ...
  for (i = 0; i < (128 * 64 / 8); i++) ssh1106_buffer[i] = *(image + i);
  }
...
void SSH1106_OLED_INIT(void (*startFunc)(void), unsigned char(*writeFunc)(unsigned char), unsigned char(*stopFunc)(void))
  ...
  ssh1106_oled_i2c_write(0xAE);




  ssh1106_oled_i2c_write(0x00 | 2);
  ssh1106_oled_i2c_write(0x10 | 0);


  ssh1106_oled_i2c_write(0xB0 | 0);


  ssh1106_oled_i2c_write(0xC0 | 8);





  ssh1106_oled_i2c_write(0x40 | 0);


  ssh1106_oled_i2c_write(0x81);
  ssh1106_oled_i2c_write(0x7F);


  ssh1106_oled_i2c_write(0xA0 | 1);


  ssh1106_oled_i2c_write(0xA6);


  ssh1106_oled_i2c_write(0xA8);
  ssh1106_oled_i2c_write(0x3F);


  ssh1106_oled_i2c_write(0xA4);


  ssh1106_oled_i2c_write(0xD3);
  ssh1106_oled_i2c_write(0x00);


  ssh1106_oled_i2c_write(0xD5);
  ssh1106_oled_i2c_write(0xF0);


  ssh1106_oled_i2c_write(0xD9);
  ssh1106_oled_i2c_write(0x22);


  ssh1106_oled_i2c_write(0xDA);
  ssh1106_oled_i2c_write(0x12);


  ssh1106_oled_i2c_write(0xDB);
...
void SSH1106_OLED_Update(void)
  ...
  for (y = 0; y < (7u + 1); y++)
    ...
    ssh1106_oled_i2c_write(0xB0 | y);
    ssh1106_oled_i2c_stop();

    ssh1106_oled_i2c_start();
    ssh1106_oled_i2c_write(0x78);
    ssh1106_oled_i2c_write(0x40);
