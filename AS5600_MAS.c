...
byte MOTOR_PID_CONTROL()
  ...
  if (oran1000 < 200) Setpoint = 3;
    ...
    uint8_t step = mid / 30; // 0 ? 20
    Setpoint = (uint32_t) MOTOR_HIZ_MAX * (40 - step) / 40;
...
byte MOTOR_START(word stop_index)
  ...
  if (MOTOR_WORK == 0)
    {
    if (abs(MAS12_NOW_MAPPED - stop_index) < MOTOR_TEKRAR_HAREKET_BASLAMA_TOLERANS) return 0;
    }
  else if (stop_index == MOTOR_STOP_INDEX) return 0;
  MOTOR_PID_RESET();
...
byte MOTOR_PROCESS(word torkVal)
  ...
  if (MOTOR_WORK)
    ...
    if (MOTOR_SUCCES)
      {
      if (abs(MAS12_NOW_MAPPED - MOTOR_LAST_SUCCESS_INDEX) > MOTOR_TEKRAR_HAREKET_BASLAMA_TOLERANS) rtrnval = MOTOR_KOL_TOPLAMA;
      }
    }
  return rtrnval;
  }
